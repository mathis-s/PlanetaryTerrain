...
namespace PlanetaryTerrain.Atmosphere
{
    public class AtmospherePostProcessing : MonoBehaviour
    {
        ...
        void Awake()
        {
            float invHeightScale = 1 / planet.heightScale;
            kLengthUnitInMeters = kBottomRadius / (planet.radius * ((invHeightScale + seaLevelHeight) /  invHeightScale));
        ...
        {
            float invHeightScale = 1 / planet.heightScale;
            m_material.SetFloat("max_terrain_radius", planet.radius * (invHeightScale + 1) /  invHeightScale);
            ...
            CAMERA_FOV = m_camera.fieldOfView;
            CAMERA_ASPECT_RATIO = m_camera.aspect;
            CAMERA_NEAR = m_camera.nearClipPlane;
            CAMERA_FAR = m_camera.farClipPlane;

            fovWHalf = CAMERA_FOV * 0.5f;

            c1 = CAMERA_NEAR * Mathf.Tan(fovWHalf * Mathf.Deg2Rad) * CAMERA_ASPECT_RATIO;
            c2 = CAMERA_NEAR * Mathf.Tan(fovWHalf * Mathf.Deg2Rad);
        }
