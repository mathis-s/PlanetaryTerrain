...
{
    public class AtmosphereMesh : MonoBehaviour
    {

        static readonly float kSunAngularRadius = 0.00935f / 2.0f;
        static readonly float kLengthUnitInMeters = 6371000f;
        ...
        public float planetSeaLevel = 0f;

        public ComputeShader m_compute;
        public Material m_material;

        private Model m_model;
        private float kBottomRadius;
        ...
        void Start()
        {
            float invHeightScale = 1f / planet.heightScale;

            mesh = ((GameObject)Instantiate(Resources.Load("scaledSpacePlanet"), transform.position, Quaternion.identity, transform)).transform;
            mesh.localScale = Vector3.one * (1f / planet.scaledSpaceFactor) * (float)topBottomRadiusRatio * planet.radius * ((invHeightScale + planetSeaLevel) / invHeightScale);
            mesh.GetComponent<MeshRenderer>().material = m_material;

            kBottomRadius = 6371000;
            ...
            double kTopRadius = kBottomRadius * topBottomRadiusRatio;
        ...
        void LateUpdate()
        {
            m_material.SetVector("earth_center", mesh.position * ((float)topBottomRadiusRatio / mesh.lossyScale.x) );
            m_material.SetFloat("mesh_radius", 1f / mesh.lossyScale.x);
